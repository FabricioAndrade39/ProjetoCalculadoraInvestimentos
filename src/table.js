...
export const createTable = (columnsArray, dataArray, tableId) => {
    if(!isNonEmptyArray(columnsArray) || !isNonEmptyArray(dataArray) || !tableId) {
        throw new Error("Para correta execução, precisamos de um array com as colunas, outro com a informação das linhas e também o id do elemento tabela selecionado");
    }
...
function createTableBody(tableReference, tableItems, columnsArray) {
    ...
    for(const [itemIndex, tableItem] of tableItems.entries()) {
        ...
        for(const tableColumn of columnsArray) {
            ...
            tableRow.innerHTML += /*html*/ `<td class = "texty-center">${formatFn(tableItem[tableColumn.accessor])}</td>`;
        }
